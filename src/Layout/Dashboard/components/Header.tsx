    ...
    return (
        <div className="h-[2.5rem] rounded-md flex items-center my-2 absolute left-4 right-4 z-50">
            <div className="flex justify-between items-center w-full">
                <a href="/">
                    <div className=" px-2 gap-2 mx-[.9rem] cursor-pointer">{is_staff ? <SeereLogo styling="mt-0 py-2" size={50}/>: (is_admin && !admin_view) && <SeereLogo styling="mt-0 py-2" size={50}/>}</div>
                </a>
                <div className="flex gap-4 items-center">
                    ...
                    <div className={`flex md:gap-8 lg:gap-12 text-gray-800 ${is_staff && "mr-5"}`}>
                        {/* {is_staff ? */}
                        {is_staff ? <div className="cursor-pointer font-bold text-red-500" onClick={handleLogout}>Logout</div>: (is_admin && !admin_view) && <div className="cursor-pointer font-bold text-red-500" onClick={handleLogout}>Logout</div>}
                        {/* <a href="/profile">
